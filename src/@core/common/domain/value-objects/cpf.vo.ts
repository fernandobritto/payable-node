...
export class Cpf extends ValueObject<string> {
  ...
  private validate() {
    if (this.value.length != 11) {
      throw new InvalidCpfError('CPF must have 11 digits, but has ' + this.value.length + ' digits')
    }
    ...
    if (firstDigit !== parseInt(this.value.charAt(9)) || secondDigit !== parseInt(this.value.charAt(10))) {
